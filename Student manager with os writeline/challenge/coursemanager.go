...
func menuCreateNewStudent(c course) error {
	...
	// Save process
	fileName := std_id + "_" + std_name + ".txt"
	data := "ID : " + std_id + "\nName : " + std_name + "\nPoint : " + strconv.Itoa(std_points)
	folder := "Student_collec"
	path := folder + "/" + fileName

	errWriteFile := os.WriteFile(path, []byte(data), 0777)
...
func writefileupdatepoints(std *student) error {
	path := std.path
	std_points := strconv.Itoa(std.points)

	fmt.Println(std.points)
	fmt.Print(reflect.TypeOf(std.points))
